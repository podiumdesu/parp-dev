...
import "./libs/rlp/RLPReader.sol";  // Assuming you have added an RLP decoding library
...
abstract contract FraudProofSPProcessor is FraudProofBase {
    ...
    function fraudProofSP(bytes memory res, bytes memory req) internal returns (bool, address, address, address) {
        (RequestBody memory request, bytes32 reqHash) = FraudProofDecoderLibrary.decodeRequest(req);
        (ResponseSPMsg memory response, bytes32 resHash) = FraudProofDecoderLibrary.decodeResponseSP(res);
        ...
        // (address sender, address recipient, uint status, ) = paychanContract.paychanSelectedArguments(request.ChannelId);
        ...
        key[0] = abi.encodePacked(response.Address);
        ...
        emit msgHash(txRootHash);
        emit emitProof(response.Proof);
        emit emitProof(key);
        ...
        // FraudProofHelper.slashWithAddresses(channelInfo.sender, channelInfo.recipient, channelInfo.witness);
