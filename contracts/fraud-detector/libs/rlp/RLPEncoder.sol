...
library RLPEncoder {
    ...
    function encodeBytes32(bytes32 data) internal pure returns (bytes memory) {
        // Convert `bytes32` to `bytes` and use `encodeBytes` for RLP encoding
        return RLPEncoder.encodeBytes(abi.encodePacked(data));
    }

    /**
     * @dev RLP encodes a list of RLP encoded byte byte strings.
     * @param self The list of RLP encoded xrbyte strings.
